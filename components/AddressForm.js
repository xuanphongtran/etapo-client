...
import { ProvinceAxios } from '@/lib/axios'
...
const AddressForm = ({ width, info }) => {
  ...
  useEffect(() => {
    // const a = ProvinceAxios.get('/provinces/getAll?limit=-1')
    if (info) {
  ...
  return (
    <div>
      ...
      <Form $width={width} onSubmit={handleSubmit(onSubmit)}>
        ...
        <Input type="text" placeholder="Quận/huyện" {...register('district', { required: true })} />
        ...
        <Input type="text" placeholder="Phường/xã" {...register('ward', { required: true })} />
